// ── Vẽ nhân vật kiểu chibi vector cho tiệm spa: khách mặc áo choàng,
//    cô nhân viên mặc đồng phục. Không phụ thuộc state game.
//
...
// khăn bông quấn đầu kiểu spa
function towelWrap(ctx, x, hy, hr, u) {
  ctx.fillStyle = '#fffaf4';
  ctx.beginPath();
  ctx.moveTo(x - hr * 1.06, hy + hr * 0.1);
  ctx.quadraticCurveTo(x - hr * 1.18, hy - hr * 1.3, x, hy - hr * 1.22);
  ctx.quadraticCurveTo(x + hr * 1.18, hy - hr * 1.3, x + hr * 1.06, hy + hr * 0.1);
  ctx.quadraticCurveTo(x, hy - hr * 0.34, x - hr * 1.06, hy + hr * 0.1);
  ctx.closePath();
  ctx.fill();
  ctx.strokeStyle = 'rgba(210,180,190,.55)';
  ctx.lineWidth = 1.3 * u;
  ctx.stroke();
  // nút khăn xoắn trên đỉnh
  ctx.fillStyle = '#fff6ee';
  ctx.beginPath(); ctx.ellipse(x + hr * 0.1, hy - hr * 1.34, hr * 0.4, hr * 0.3, 0.3, 0, 7); ctx.fill();
  ctx.strokeStyle = 'rgba(210,180,190,.5)';
  ctx.beginPath(); ctx.ellipse(x + hr * 0.1, hy - hr * 1.34, hr * 0.4, hr * 0.3, 0.3, 0, 7); ctx.stroke();
  ctx.beginPath();
  ctx.moveTo(x - hr * 0.2, hy - hr * 1.16);
  ctx.quadraticCurveTo(x + hr * 0.15, hy - hr * 1.44, x + hr * 0.48, hy - hr * 1.2);
  ctx.stroke();
}

function hairFront(ctx, x, hy, hr, look, u) {
  if (look.towel) { towelWrap(ctx, x, hy, hr, u); return; }
  const col = look.hairColor;
  const s = look.hair;
  ctx.fillStyle = col;
...
function outfit(ctx, x, by, u, look, o) {
  ...
  if (dress === 'robe') {
    ...
    ctx.quadraticCurveTo(x - 17 * u, waistY - 10 * u, x - 14 * u, waistY + 2 * u);
    ctx.quadraticCurveTo(x - 16 * u, hemY + 4 * u, x - 14 * u, hemY + 9 * u);
    ctx.lineTo(x + 14 * u, hemY + 9 * u);
    ctx.quadraticCurveTo(x + 16 * u, hemY + 4 * u, x + 14 * u, waistY + 2 * u);
    ctx.quadraticCurveTo(x + 17 * u, waistY - 10 * u, x + 15 * u, shoulderY - 1 * u);
    ctx.closePath();
    ctx.fill();
    outline(ctx, dcol, u, 0.8);
    // vạt áo chéo bên phải
    ctx.fillStyle = shade(dcol, 1.06);
    ctx.beginPath();
    ctx.moveTo(x + 15 * u, shoulderY - 1 * u);
    ctx.quadraticCurveTo(x + 16 * u, waistY - 8 * u, x + 14 * u, hemY + 8 * u);
    ctx.lineTo(x - 2 * u, hemY + 8 * u);
    ctx.quadraticCurveTo(x - 1 * u, waistY - 4 * u, x - 9 * u, shoulderY + 2 * u);
    ctx.closePath();
    ctx.fill();
    // cổ áo bản to
    ctx.fillStyle = 'rgba(255,255,255,.9)';
    ctx.beginPath();
    ctx.moveTo(x - 10 * u, shoulderY - 2 * u);
    ctx.quadraticCurveTo(x - 5 * u, shoulderY + 12 * u, x + 1 * u, waistY - 2 * u);
    ctx.quadraticCurveTo(x + 4 * u, shoulderY + 10 * u, x + 10 * u, shoulderY - 2 * u);
    ctx.quadraticCurveTo(x, shoulderY + 4 * u, x - 10 * u, shoulderY - 2 * u);
    ctx.closePath();
    ctx.fill();
    // dây lưng + nơ
    ctx.fillStyle = shade(dcol, 0.68);
    rr(ctx, x - 14.5 * u, waistY - 3 * u, 29 * u, 4.6 * u, 2.3 * u); ctx.fill();
    ctx.fillStyle = shade(dcol, 0.6);
    ctx.beginPath(); ctx.ellipse(x - 3.6 * u, waistY - 0.6 * u, 3.6 * u, 2.8 * u, -0.5, 0, 7); ctx.fill();
    ctx.beginPath(); ctx.ellipse(x + 3.6 * u, waistY - 0.6 * u, 3.6 * u, 2.8 * u, 0.5, 0, 7); ctx.fill();
    ctx.beginPath(); ctx.arc(x, waistY - 0.6 * u, 1.8 * u, 0, 7); ctx.fill();
    return { shoulderY };
  }
...
// vẽ riêng phần đầu — dùng cho khách đang nằm mát-xa, đắp mặt nạ,
// hoặc chỉ nhô đầu qua cửa sổ phòng xông hơi
export function drawHeadOnly(ctx, x, y, r, look, o) {
  o = o || {};
  const skin = look.skin || '#ffdfc4';
  const u = r / 20;
  hairBack(ctx, x, y, r, look, u, y + r * 2);
  // tai
  ctx.fillStyle = shade(skin, 0.95);
  for (const sg of [-1, 1]) {
    ctx.beginPath();
    ctx.ellipse(x + sg * r * 0.97, y + r * 0.22, r * 0.16, r * 0.2, 0, 0, 7);
    ctx.fill();
  }
  ctx.fillStyle = o.mask ? '#eafaf0' : skin;
  ctx.beginPath(); ctx.ellipse(x, y, r * 0.97, r, 0, 0, 7); ctx.fill();
  ctx.strokeStyle = shade(o.mask ? '#cfe8d8' : skin, 0.9);
  ctx.lineWidth = 1.1 * u;
  ctx.stroke();

  if (o.mask) {
    // mặt nạ dưa leo trên mắt + lớp kem xanh mát
    ctx.fillStyle = 'rgba(150,220,170,.55)';
    ctx.beginPath(); ctx.ellipse(x, y + r * 0.12, r * 0.86, r * 0.72, 0, 0, 7); ctx.fill();
    for (const sg of [-1, 1]) {
      ctx.fillStyle = '#a8dd8a';
      ctx.beginPath(); ctx.ellipse(x + sg * r * 0.44, y + r * 0.12, r * 0.27, r * 0.2, 0, 0, 7); ctx.fill();
      ctx.fillStyle = '#c8ecab';
      ctx.beginPath(); ctx.ellipse(x + sg * r * 0.44, y + r * 0.12, r * 0.16, r * 0.11, 0, 0, 7); ctx.fill();
    }
    ctx.strokeStyle = '#c2506a';
    ctx.lineWidth = Math.max(1.4, r * 0.09);
    ctx.beginPath(); ctx.arc(x, y + r * 0.5, r * 0.2, Math.PI * 0.2, Math.PI * 0.8); ctx.stroke();
  } else {
    blush(ctx, x, y, r);
    eyes(ctx, x, y, r, o.awake ? 'calm' : 'happy', u);
    mouth(ctx, x, y, r, o.awake ? 'calm' : 'happy', u);
  }
  hairFront(ctx, x, y, r, look, u);
  accessory(ctx, x, y, r, look, u);

  if (o.sweat) {
    ctx.fillStyle = 'rgba(120,190,255,.9)';
    for (const [dx, dy, s2] of [[1.02, -0.42, 1], [-1.0, -0.1, 0.8]]) {
      ctx.beginPath();
      ctx.ellipse(x + r * dx, y + r * dy + Math.sin(Date.now() / 500 + dx) * r * 0.06,
        r * 0.12 * s2, r * 0.17 * s2, 0.3, 0, 7);
      ctx.fill();
    }
  }
}

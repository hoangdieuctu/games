// ── Hạt hiệu ứng: lấp lánh, xu bay, hơi nóng, khói, tim, chữ nổi ──
...
export function coin(x, y) {
  G.particles.push({ t: 'coin', x, y, sx: x, sy: y, life: 0.75, max: 0.75, dx: G.W - 190, dy: 34 });
}
export function steam(x, y) {
  G.particles.push({ t: 'steam', x, y, vx: (Math.random() - 0.5) * 14, vy: -34 - Math.random() * 24, life: 1.5, max: 1.5, sz: (8 + Math.random() * 8) * G.K });
}
...
export function textPop(x, y, txt, size, col) {
  G.particles.push({ t: 'text', x, y, vx: 0, vy: -46, life: 1.2, max: 1.2, txt, sz: (size || 22) * G.K, col: col || '#c2557a' });
}
